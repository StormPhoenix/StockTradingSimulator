...
export class GenericWorkerThreadPool extends EventEmitter {
   ...
   * 注册任务适配器
   */
  public registerTaskAdapter<TRequest, TResponse>(
   ...
   * 提交任务（使用适配器）
   */
  public async submitTask<TRequest, TResponse>(
    taskType: string,
   ...
   * 提交通用任务
   */
  public async submitGenericTask(request: GenericTaskRequest): Promise<string> {
    // 生成任务ID（如果没有提供）
    if (!request.taskId) {

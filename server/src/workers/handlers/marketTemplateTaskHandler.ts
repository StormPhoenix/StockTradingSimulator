...
export class MarketTemplateTaskHandler implements TaskHandler<MarketTemplateRequest, MarketTemplateResponse> {
  ...
  public async handleTask(request: GenericTaskRequest<MarketTemplateRequest>): Promise<MarketTemplateResponse> {
    ...
    try {
      // 发送初始进度
      this.sendProgress(taskId, MarketTemplateStage.DATABASE_READING, 0, 'Connecting to database...');

      // 连接数据库
      await this.connectToDatabase();
      this.sendProgress(taskId, MarketTemplateStage.DATABASE_READING, 10, 'Database connected, reading templates...');

      // 读取交易所模板
      const exchange = await this.readExchangeTemplate(templateId);
      this.sendProgress(taskId, MarketTemplateStage.DATABASE_READING, 30, 'Exchange template loaded');

      // 读取交易员模板
      const traders = await this.readTraderTemplates(templateId);
      this.sendProgress(taskId, MarketTemplateStage.DATABASE_READING, 60, `${traders.length} trader templates loaded`);

      // 读取股票模板
      const stocks = await this.readStockTemplates(templateId);
      this.sendProgress(taskId, MarketTemplateStage.DATABASE_READING, 90, `${stocks.length} stock templates loaded`);

      // 数据处理阶段
      this.sendProgress(taskId, MarketTemplateStage.DATA_PROCESSING, 95, 'Processing template data...');

      // 验证数据完整性
      this.validateTemplateData(exchange, traders, stocks);

      // 发送完成进度
      this.sendProgress(taskId, MarketTemplateStage.COMPLETE, 100, 'Template data ready', {
      ...
      this.sendError(taskId, {
        code: 'MARKET_TEMPLATE_ERROR',
        message: errorMessage,
        details: { templateId, userId }
      });
   ...
   * 发送进度更新
   */
  public sendProgress(taskId: string, stage: string, percentage: number, message: string, details?: any): void {
    const progress = {
      taskId,
   ...
   * 发送错误
   */
  public sendError(taskId: string, error: TaskError): void {
    const response = {
      taskId,
      taskType: this.taskType,
      timestamp: new Date(),
